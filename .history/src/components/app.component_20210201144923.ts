...
const testKey = localStorage?.stripeAngularKey || "pk_test_5JZuHhxsinNGc5JanVWWKSKq"
...
@Component({
  ...
  async save(): Promise<stripe.Stripe>{
    if (this.saveKeyLocally) {
      ...
      this.stripeAngular.key = this.tempPublishableKey || this.publishableKey
    }

    this.privateKey = this.tempPrivateKey;
    if (this.savePrivateKeyLocally) {
      // localStorage.stripeAngularPrivateKey = this.privateKey;
      this.stripeAngular.privateKey = this.privateKey
    }

    localStorage.stripeAngular = JSON.stringify(this.stripeAngular)

    this.publishableKey = this.tempPublishableKey || this.publishableKey;
